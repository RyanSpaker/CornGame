        ...
        + SubOne,
{
    ...
    pub fn calculate_overlap<'a>(set_a: &'a Self, set_b: &'a Self) -> Vec<(i32, &'a T)> {
        ...
        while a_index < set_a.endpoints.len() || b_index < set_b.endpoints.len() {
            if b_index >= set_b.endpoints.len() {
                ...
                overlap.push((
                    (1 - a_index as i32 % 2) * 2 - 1 + (1 - b_index as i32 % 2) * 2 - 1,
                    &set_a.endpoints[a_index],
                ));
                a_index += 1;
                b_index += 1;
            }
    ...
    pub fn calculate_overlap_vec<'a>(sets: &'a Vec<Self>) -> Vec<(i32, &'a T)> {
        ...
        while indices
            .iter()
            .enumerate()
            .any(|(i, index)| *index < sets[i].endpoints.len())
        {
            let valid_indices: Vec<usize> = indices
                .iter()
                .enumerate()
                .filter_map(|(i, index)| {
                    if *index >= sets[i].endpoints.len() {
                        return None;
                    }
                    return Some(i);
                })
    ...
    pub fn get_continuos(&self, length: T) -> Option<Self> {
        ...
        return Some(Self::simple(
            &self.endpoints[mindex],
            &(self.endpoints[mindex].to_owned() + length),
        ));
    }
